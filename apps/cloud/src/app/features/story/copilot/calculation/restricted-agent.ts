...
export async function createRestrictedMeasureWorker({ llm, tools }: { llm: ChatOpenAI; tools: DynamicStructuredTool[] }) {
  const systemPrompt = `You are a data analyst. Please create a measure that aggregate values based on restrictions imposed by dimension members.` +
