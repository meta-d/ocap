...
import { INDICATOR_WORKER_NAME, PLANNER_NAME, SUPERVISOR_NAME } from './types'
...
export async function createIndicatorGraph({
  ...
  const createIndicator = await createIndicatorWorker({ llm, createIndicatorTool, pickCubeTool, copilotRoleContext })
...
async function createIndicatorWorker({ llm, createIndicatorTool, pickCubeTool, copilotRoleContext }) {
  const systemPrompt = `你是一名 BI 指标体系管理的业务专家，请根据指定的 Cube 信息和需求描述转成相应的参数调用 create_indicator 函数进行创建新指标。` +
  `\n{role}\n` +
