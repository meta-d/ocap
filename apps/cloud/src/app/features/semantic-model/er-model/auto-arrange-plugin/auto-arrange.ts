...
export class AutoArrange {
  constructor(public editor, public margin, public depth, public vertical, public offset) {}
  ...
  getNodes(node, type = 'output') {
    ...
    for (let io of node[key].values())
      for (let connection of io.connections.values()) nodes.push(connection[type === 'input' ? 'output' : 'input'].node)
  ...
  arrange(
    ...
    { margin = this.margin, vertical = this.vertical, depth = this.depth, offset = this.offset, skip, substitution }
  ) {
