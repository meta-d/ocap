  ...
  return json => {
    ...
    if (isChatCompletionChunk(json)) {
      ...
      if (
        ...
        (json.output?.finish_reason === 'function_call' ||
          json.output?.finish_reason === 'stop')
      ) {
        isFunctionStreamingIn = false; // Reset the flag
        return '"}}';
      } else if (
        isFunctionStreamingIn &&
        json.output?.finish_reason === 'tool_calls'
      ) {
        isFunctionStreamingIn = false; // Reset the flag
        return '"}}]}';
      }
    ...
    const text = trimStartOfStream(
      isChatCompletionChunk(json) && json.output.delta
        ? json.output.delta
        : isCompletion(json)
        ? json.output.text
        : '',
...
function createFunctionCallTransformer(
  ...
  const functionCallMessages: CreateMessage[] =
    ...
    async flush(controller): Promise<void> {
      try {
        if (
          ...
          const openAIStream = DashScopeStream(functionResponse, {
            ...filteredCallbacks,
