...
import { multiORMCreateQueryBuilder } from '../orm/query-builder/query-builder.factory';
...
export abstract class CrudService<T extends BaseEntity> implements ICrudService<T> {
	...
	*/
	 ...
	 */
	public async findOneOrFail(id: string | IFindOneOptions<T> | IFindWhereOptions<T>, options?: IFindOneOptions<T>): Promise<ITryRequest<T>> {
		if (options) {
			return this.findOneOrFailByIdString(id as T['id'], options);
		}
		if (typeof id === 'string') {
			return this.findOneOrFailByIdString(id as T['id'], options);
		}
		if ((<IFindOneOptions<T>>id).where) {
			return this.findOneOrFailByOptions(id as IFindOneOptions<T>);
		}
		return this.findOneOrFailByWhereOptions(id as IFindWhereOptions<T>);
	}
	...
	*/
	 ...
	 * @deprecated back compatibility
	 */
	public async findOne(id: T['id'] | IFindWhereOptions<T> | IFindOneOptions<T>, options?: FindOneOptions<T>): Promise<T> {
		if (options) {
			return this.findOneByIdString(id as T['id'], options);
		}
		if (typeof id === 'string') {
			return this.findOneByIdString(id as T['id'], options);
		}
		if ((<IFindOneOptions<T>>id).where) {
			return this.findOneByOptions(id as IFindOneOptions<T>);
		}
		return this.findOneByWhereOptions(id as IFindWhereOptions<T>);
	}
	/**
	 * Finds first entity by a given find options.
