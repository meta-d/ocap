...
import { IPagination, IUser, PermissionsEnum } from '@metad/contracts';
...
export abstract class TenantAwareCrudService<T extends TenantBaseEntity> extends CrudService<T> implements ICrudService<T> {
	...
	constructor(public repository: Repository<T>, mikroOrmRepository?: MikroOrmBaseEntityRepository<T>) {
		super(repository, mikroOrmRepository);
	}
	...
	*/
	...
	public async findOne(id: T['id'] | IFindWhereOptions<T> | IFindOneOptions<T>, options?: FindOneOptions<T>): Promise<T> {
		...
		if ((<FindOneOptions<T>>id).where) {
			return this.findOneByOptions(id as FindOneOptions<T>);
		}
		return this.findOneByWhereOptions(id as FindOptionsWhere<T>);
	}
