...
//             options.headers = {};
//             if (username) options.headers.Authorization = "Basic " + (new ArrayBuffer(username + ":" + (password || ""))).toString("base64");
...
//                 client
//             ;
//             options.headers["Content-Length"] = new ArrayBuffer(data).byteLength;
...
function _ajax(options, agent) {
  ...
  if (agent) {
    return agent.request(options.semanticModel, options).then((response) => {
      const xmlParser = new DOMParser() //创建 DOMParser 对象
      response.request = {
        response: response.data,
        responseText: response.data,
        responseXML: xmlParser.parseFromString(response.data, 'text/xml'),
        ...
        criteria =
          tagName === '*'
            ...
            : function (node) {
                return (
                  node.nodeType === 1 &&
                  (node.namespaceURI === '' ? '' : node.prefix + ':') + node.nodeName === tagName
                )
              }
      _getElements(node, list, criteria)
      return list
...
let _getElementText = function (el) {
...
function _xjs(xml) {
  ...
  function unescapeEntities(text) {
    return text.replace(/&((\w+)|#(x?)([0-9a-fA-F]+));/g, function (
      match,
      g1,
      g2,
      g3,
...
*/
   ...
   *   </dl>
   *   @method addListener
   *   @param listener An object that defines the events and the notification function to be called, or an array of such objects.
   */
  addListener: function () {
    ...
    switch (n) {
      case 0:
        Xmla.Exception._newError('NO_EVENTS_SPECIFIED', 'Xmla.addListener', null)._throw()
      case 1:
        var arg = arguments[0]
        if (_isObj(arg)) {
          var events
          if (_isArr(arg)) this._addListeners(arg)
            ...
            if (!_isArr(events)) {
              Xmla.Exception._newError('WRONG_EVENTS_FORMAT', 'Xmla.addListener', arg)._throw()
            }
            var i
            n = events.length
        ...
        } else Xmla.Exception._newError('WRONG_EVENTS_FORMAT', 'Xmla.addListener', arg)._throw()
        break
      case 2:
      case 3:
   ...
   *   @return The result of the invoking the XML/A method. For an asynchronous request, the return value is not defined. For synchronous requests, <code>Discover</code> requests return an instance of a <code>Xmla.Rowset</code>, and <code>Execute</code> results return an instance of a <code>Xmla.Dataset</code>.
   */
  request: function (options: MDOptions) {
    let ex
    const xmla = this
    ...
    const soapMessage = this.getXmlaSoapMessage(options)
    ...
    if (
      ...
      ajaxOptions.headers = merge(ajaxOptions.headers, {
        Accept: 'text/xml, application/xml, application/soap+xml',
        'Content-Type': 'text/xml',
      })
      ...
      // _ajax(ajaxOptions, this.options.agent)
        .then((body: string) => {
          ...
          options.xhr = {
            response: body,
            responseText: body,
            responseXML: xmlParser.parseFromString(body, 'text/xml'),
            status: 200,
            statusText: 'OK'
          }
  ...
  _requestSuccess: function (request) {
    ...
    try {
      ...
      if (soapFault.length) {
        ...
        const faultDetail = _getElementsByTagName(soapFault, 'detail')[0]

        let error = faultDetail.textContent
        const errors = _getElementsByTagName(faultDetail, 'Error')
        if (errors) {
          for(let i = 0; i < errors.length; i++) {
            error = `${error}\n${_getAttribute(errors[i], 'Description')}`
          }
        }
        if (!error) {
          error = _getElementsByTagName(soapFault, 'faultstring')[0].childNodes[0].data
        }
    ...
    if (request.exception) {
      ...
      if (request.success)
        request.success.call(request.scope ? request.scope : null, this, request, response)
      if (request.callback)
        request.callback.call(
          request.scope ? request.scope : null,
          Xmla.EVENT_SUCCESS,
...
*/
   ...
   *   @param object options An object whose properties convey the options for the XML/A <code>Execute</code> request.
   *   @return The result of the invoking the XML/A <code>Execute</code> method. For an asynchronous request, the return value is not defined. For synchronous requests, an instance of a <code>Xmla.Rowset</code> that represents the multi-dimensional result set of the MDX query.
   */
 ...
 *   @class Xmla.Rowset
 *   @constructor
 *   @param node The responseXML result returned by server in response to a <code>Discover</code> request.
 *   @param requestTtype The requestType identifying the particular schema rowset to construct. This facilitates implementing field getters for a few complex types.
 *   @param xmla The Xmla instance that created this Rowset. This is mainly used to allow the Rowset to access the options passed to the Xmla constructor.
 */
...
Xmla.Rowset.prototype = {
    ...
    if (rowSchema) {
      ...
      for (i = 0; i < numChildren; i++) {
        ...
        if (type === null && this._row) {
          ...
          if (val.length) {
            type = _getAttributeNS(val[0], _xmlnsSchemaInstance, _xmlnsSchemaInstancePrefix, 'type')
          }
        }
   ...
   *
   *   @method getFields
   *   @return An (ordered) array of field definition objects.
   */
  getFields: function () {
    var f = [],
      i,
   ...
   *   @return An (ordered) array of field names.
   */
...
*   @method hasMoreRows
*   @return true in case there are more rows to traverse. false if all rows have been traversed.
*/
   ...
   *   @return true if all rows were itereated. If the callback returns false, iteration stops and false is returned.
   */
   ...
   *   @return The <code>fieldDef</code> object that matches the argument.
   */
  fieldDef: function (name) {
    var field = this.fields[name]
    if (!field) Xmla.Exception._newError('INVALID_FIELD', 'Xmla.Rowset.fieldDef', name)._throw()
    return field
  },
  /**
   ...
   *   @method fieldIndex
   *   @param name The name of the field for which you want to retrieve the index.
   ...
   *   @method fieldVal
   *   @param name The name or the index of the field for which you want to retrieve the value.
   *   @return From the current row, the value of the field that matches the argument.
   */
 ...
 *   @class Xmla.Dataset
 *   @constructor
 *   @param doc The responseXML result returned by server in response to a <code>Execute</code> request.
 */
...
Xmla.Dataset.prototype = {
  ...
  _initRoot: function (doc) {
    ...
    else Xmla.Exception._newError('ERROR_PARSING_RESPONSE', 'Xmla.Dataset._initData', doc)._throw()
  },
  _initAxes: function () {
    var i,
      axis,
   ...
   *   @return The <code><a href="Xmla.Dataset.Axis.html#class_Axis">Xmla.Dataset.Axis</a></code> object that corresponds to the argument.
   */
  getAxis: function (nameOrIndex): Axis {
    if (nameOrIndex === Xmla.Dataset.AXIS_SLICER) return this._slicer
    var axis = this._getAxis(nameOrIndex)
    if (!axis)
      Xmla.Exception._newError('INVALID_AXIS', 'Xmla.Dataset.getAxis', nameOrIndex)._throw()
    return axis
  },
  /**
   ...
   *   @return The column <code><a href="Xmla.Dataset.Axis.html#class_Axis">Xmla.Dataset.Axis</a></code> object.
   */
   ...
   *   @method getRowAxis
   *   @return The row <code><a href="Xmla.Dataset.Axis.html#class_Axis">Xmla.Dataset.Axis</a></code> object.
   */
  getRowAxis: function () {
   ...
   *   @method getPageAxis
   *   @return The page <code><a href="Xmla.Dataset.Axis.html#class_Axis">Xmla.Dataset.Axis</a></code> object.
   */
   ...
   *   @return The section <code><a href="Xmla.Dataset.Axis.html#class_Axis">Xmla.Dataset.Axis</a></code> object.
   */
   ...
   *   @return The chapter <code><a href="Xmla.Dataset.Axis.html#class_Axis">Xmla.Dataset.Axis</a></code> object.
   */
   ...
   *   @return The slicer <code><a href="Xmla.Dataset.Axis.html#class_Axis">Xmla.Dataset.Axis</a></code> object.
   */
   ...
   *   @method getCellset
   *   @return The <code><a href="Xmla.Dataset.Cellset.html#class_Cellset">Xmla.Dataset.Cellset</a></code> object.
   */
  getCellset: function () {
...
Xmla.Dataset.Axis.prototype = {
  ...
  _initHierarchies: function (_axisInfoNode) {
    ...
    for (i = 0; i < numHierarchies; i++) {
      ...
      for (j = 0; j < numPropertyNodes; j++) {
        ...
        // 获取属性的 name, 为后续数据转换使用
        nodeName = propertyNode.nodeName
        properties[nodeName] = {
          name: _getAttribute(propertyNode, 'name')
        }
        const uniqueName = _getAttribute(propertyNode, 'name')
        if (memberProperties[nodeName]) {
          memberProperties[nodeName].uniqueName = uniqueName
          continue
        }
  ...
  _initMembers: function () {
    ...
    if (!memberSchema)
      Xmla.Exception._newError('ERROR_PARSING_RESPONSE', 'Xmla.DataSet.Axis', root)._throw()
  ...
  _getMembers: function () {
    ...
    return _getElementsByTagNameNS(this._tuples[this._tupleIndex], _xmlnsDataset, '', 'Member')
  },
  /**
   ...
   *   @param callback A function that will be called for each hierarchy. The hierarchy is passed as an object as the first argument to the callback. For the structure of the hierarchy object, see <a href="#method_hierarchy">hierarchy()</a>.
   ...
   *   @param callback A function that will be called for each tuple. The tuple is passed as an object as the first argument to the callback.
   *   @param object scope The object that will be used as scope when executing the callback function. If this is undefined or <code>null</code>, the Axis' <code>this</code> pointer will be used.
   ...
   *   @return An array of hierarchies contained in this Axis.
   **/
   ...
   *   @return An array of names of the hierarchies contained in this Axis.
   **/
   ...
   *   @return The name of the hierarchy specified by the argument index, or the name of the current hierarchy if the index argument is omitted.
   */
   ...
   *   @return The hierarchy specified by the argument index or name, or the current hierarchy if the argument is omitted.
   */
   ...
   */
  member: function (hierarchyIndexOrName: any) {
    if (_isUnd(hierarchyIndexOrName)) index = this._hierarchyIndex
    ...
    switch (typeof hierarchyIndexOrName) {
      ...
      case 'number':
        if (
          hierarchyIndexOrName !== parseInt(String(hierarchyIndexOrName), 10) ||
          hierarchyIndexOrName >= this.numHierarchies
        )
          Xmla.Exception._newError(
            'INVALID_HIERARCHY',
  ...
  _member: function (index) {
    ...
    for (i = 0; i < n; i++) {
      ...
      valueConverter = property.converter || _getValueConverter(_getAttribute(el, 'type'))
      // member[property.name] = valueConverter(_getElementText(el))
      member[hierarchy[el.nodeName].name] = valueConverter(_getElementText(el))
    }
    return member
  },
  /**
   ...
   *   @return An array of members that represents the current tuple.
   */
  readAsArray: function (array) {
    if (!array) array = []
    var i,
   ...
   *   @method fetchAsArray
   *   @param array {array} An existing array to store the members in. If omitted, a new array is returned.
   *   @return An array of members that represents the current tuple, or <code>false</code> if there are no more tuples.
   */
  fetchAsArray: function (array) {
   ...
   *   @return An array of arrays representing all tuples that belong to this axis.
   **/
  fetchAllAsArray: function (rows) {
    var row
    if (!rows) rows = []
...
Xmla.Dataset.Cellset.prototype = {
  ...
  _initCellset: function () {
    ...
    if (!cellSchema)
      Xmla.Exception._newError('ERROR_PARSING_RESPONSE', 'Xmla.Dataset.Cellset', root)._throw()
   ...
   *   @return returns the value of the specified property of the current cell.
   */
  cellProperty: function (propertyName) {
    var text,
      type,
      valueConverter,
      valueEl = _getElementsByTagNameNS(this._cellNode, _xmlnsDataset, '', propertyName),
      value
    if (valueEl.length) {
      valueEl = valueEl[0]
      ...
      if (!valueConverter) {
        type = _getAttributeNS(valueEl, _xmlnsSchemaInstance, _xmlnsSchemaInstancePrefix, 'type')
        valueConverter = _getValueConverter(type)
      }
