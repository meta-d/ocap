...
export class DisplayBehaviourComponent implements OnInit, OnChanges {
  ...
  default: any = []
  ngOnInit() {
    this.value = splitByHighlight(this.option.value, this.highlight)
    this.text = splitByHighlight(this.option.label, this.highlight)
    this.default = splitByHighlight(this.option.label || this.option.value, this.highlight)
  }
